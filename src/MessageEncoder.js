...
class MessageEncoder {
  ...
  _error(id, error, message) {
    if (typeof error === 'string') {
      throw new BrokerError({ id: id, code: null, message: error + ' ' + message })
    } else {
      throw new BrokerError({ id: id, code: error.code, message: error.message + ' ' + message })
    }
  }
  ...
  sendV100APIHeader() {
    ...
    let version = this._encode(
      'v' + (MIN_VERSION < MAX_VERSION ? MIN_VERSION + '..' + MAX_VERSION : MIN_VERSION)
    )
    version.splice(-1)
  ...
  cancelScannerSubscription(tickerId) {
    if (this._serverVersion < 24) {
      return this._error(
        tickerId,
        BROKER_ERRORS.UPDATE_TWS,
        'It does not support API scanner subscription.'
      )
    }
    let version = 1
    let buffer = [
      this._encode(OUTGOING.CANCEL_SCANNER_SUBSCRIPTION),
  ...
  reqScannerSubscription(
    ...
    if (
      this._serverVersion < MIN_SERVER_VER.SCANNER_GENERIC_OPTS &&
  ...
  reqMktData(tickerId, contract, genericTickList, snapshot, regulatorySnapshot, mktDataOptions) {
    ...
    if (this._serverVersion < MIN_SERVER_VER.REQ_MKT_DATA_CONID) {
      return this._error(tickerId, BROKER_ERRORS.UPDATE_TWS, 'It does not support conId parameter.')
    }
    ...
    let version = 11
    let buffer = [
      this._encode(OUTGOING.REQ_MKT_DATA),
      this._encode(version),
      this._encode(tickerId)
    ]
  ...
  cancelHistoricalData(tickerId) {
    ...
    let version = 1
    let buffer = [
      this._encode(OUTGOING.CANCEL_HISTORICAL_DATA),
      this._encode(version),
      this._encode(tickerId)
    ]
  ...
  cancelRealTimeBars(tickerId) {
    ...
    let version = 1
    let buffer = [
      this._encode(OUTGOING.CANCEL_REAL_TIME_BARS),
      this._encode(version),
      this._encode(tickerId)
    ]
  ...
  reqHeadTimestamp(reqId, contract, whatToShow, useRTH, formatDate) {
    if (this._serverVersion < MIN_SERVER_VER.REQ_HEAD_TIMESTAMP) {
      return this._error(reqId, BROKER_ERRORS.UPDATE_TWS, 'It does not support reqHeadTimeStamp')
    }
    return [
  ...
  cancelHeadTimestamp(reqId) {
    ...
    return [this._encode(OUTGOING.CANCEL_HEAD_TIMESTAMP), this._encode(reqId)]
  }

  reqRealTimeBars(tickerId, contract, barSize, whatToShow, useRTH, realTimeBarsOptions) {
    if (this._serverVersion < MIN_SERVER_VER.REAL_TIME_BARS) {
      return this._error(tickerId, BROKER_ERRORS.UPDATE_TWS, 'It does not support real time bars.')
    }
    if (this._serverVersion < MIN_SERVER_VER.TRADING_CLASS) {
      if (!isEmpty(contract.tradingClass) || contract.conId > 0) {
        return this._error(
    ...
    let buffer = [
      this._encode(OUTGOING.REQ_REAL_TIME_BARS),
      this._encode(version),
      this._encode(tickerId)
    ]
    // send contract fields
  ...
  reqMktDepth(tickerId, contract, numRows, isSmartDepth, mktDepthOptions) {
    ...
    // send req mkt data msg
    let buffer = [
      this._encode(OUTGOING.REQ_MKT_DEPTH),
      this._encode(version),
      this._encode(tickerId)
    ]
    // send contract fields
  ...
  cancelMktData(tickerId) {
    let version = 1
    let buffer = [
      this._encode(OUTGOING.CANCEL_MKT_DATA),
      this._encode(version),
      this._encode(tickerId)
    ]
  ...
  cancelMktDepth(tickerId, isSmartDepth) {
    ...
    let version = 1
    let buffer = [
      this._encode(OUTGOING.CANCEL_MKT_DEPTH),
      this._encode(version),
      this._encode(tickerId)
    ]
  ...
  exerciseOptions(tickerId, contract, exerciseAction, exerciseQuantity, account, override) {
    ...
    let buffer = [
      this._encode(OUTGOING.EXERCISE_OPTIONS),
      this._encode(version),
      this._encode(tickerId)
    ]
    // send contract fields
  ...
  placeOrder(id, contract, order) {
    if (this._serverVersion < MIN_SERVER_VER.SCALE_ORDERS) {
      if (
        ...
        return this._error(id, BROKER_ERRORS.UPDATE_TWS, 'It does not support Scale orders.')
      }
    }
    ...
    if (this._serverVersion < MIN_SERVER_VER.WHAT_IF_ORDERS) {
      if (order.whatIf) {
        return this._error(id, BROKER_ERRORS.UPDATE_TWS, 'It does not support what-if orders.')
      }
    }
    if (this._serverVersion < MIN_SERVER_VER.DELTA_NEUTRAL) {
    ...
    if (this._serverVersion < MIN_SERVER_VER.ALGO_ORDERS) {
      if (!isEmpty(order.algoStrategy)) {
        return this._error(id, BROKER_ERRORS.UPDATE_TWS, 'It does not support algo orders.')
      }
    }
    if (this._serverVersion < MIN_SERVER_VER.NOT_HELD) {
      if (order.notHeld) {
        return this._error(id, BROKER_ERRORS.UPDATE_TWS, 'It does not support notHeld parameter.')
      }
    }
    if (this._serverVersion < MIN_SERVER_VER.SEC_ID_TYPE) {
      if (!isEmpty(contract.secIdType) || !isEmpty(contract.secId)) {
    ...
    if (this._serverVersion < MIN_SERVER_VER.PLACE_ORDER_CONID) {
      if (contract.conId > 0) {
        return this._error(id, BROKER_ERRORS.UPDATE_TWS, 'It does not support conId parameter.')
      }
    }
    ...
    if (this._serverVersion < MIN_SERVER_VER.SSHORTX) {
      if (Array.isArray(contract.comboLegs)) {
        contract.comboLegs.forEach(
          comboLeg =>
            comboLeg.exemptCode !== -1 && this._error('It does not support exemptCode parameter.')
        )
      }
    }
    if (this._serverVersion < MIN_SERVER_VER.HEDGE_ORDERS) {
      if (!isEmpty(order.hedgeType)) {
        return this._error(id, BROKER_ERRORS.UPDATE_TWS, 'It does not support hedge orders.')
      }
    }
    ...
    if (this._serverVersion < MIN_SERVER_VER.ALGO_ID && !isEmpty(order.algoId)) {
      return this._error(id, BROKER_ERRORS.UPDATE_TWS, 'It does not support algoId parameter.')
    }
    ...
    if (this._serverVersion < MIN_SERVER_VER.EXT_OPERATOR && !isEmpty(order.extOperator)) {
      return this._error(id, BROKER_ERRORS.UPDATE_TWS, 'It does not support ext operator')
    }

    if (
      this._serverVersion < MIN_SERVER_VER.SOFT_DOLLAR_TIER &&
      order.softDollarTier &&
        (!isEmpty(order.softDollarTier.name) || !isEmpty(order.softDollarTier.value))
    ) {
      return this._error(id, BROKER_ERRORS.UPDATE_TWS, 'It does not support soft dollar tier.')
    }
    ...
    if (
      this._serverVersion < MIN_SERVER_VER.DECISION_MAKER &&
    ...
    if (
      this._serverVersion < MIN_SERVER_VER.MIFID_EXECUTION &&
    ...
    if (
      this._serverVersion < MIN_SERVER_VER.AUTO_PRICE_FOR_HEDGE &&
    ...
    if (this._serverVersion < MIN_SERVER_VER.D_PEG_ORDERS && order.discretionaryUpToLimitPrice) {
      return this._error(id, BROKER_ERRORS.UPDATE_TWS, 'It does not support D-Peg orders.')
    }
    ...
    else buffer.push(this._encode(parseInt(order.totalQuantity, 10)))
  ...
  reqAccountUpdates(subscribe, acctCode) {
    ...
    if (this._serverVersion >= 9) {
      return [
        this._encode(OUTGOING.REQ_ACCOUNT_DATA),
        this._encode(version),
        this._encode(subscribe),
  ...
  setServerLogLevel(logLevel) {
    let version = 1
    return [
      this._encode(OUTGOING.SET_SERVER_LOGLEVEL),
      this._encode(version),
      this._encode(logLevel)
    ]
  }
  ...
  reqAutoOpenOrders(bAutoBind) {
    let version = 1
    return [
      this._encode(OUTGOING.REQ_AUTO_OPEN_ORDERS),
      this._encode(version),
      this._encode(bAutoBind)
    ]
  }
  ...
  replaceFA(faDataType, xml) {
    ...
    let version = 1
    return [
      this._encode(OUTGOING.REPLACE_FA),
      this._encode(version),
      this._encode(faDataType),
      this._encode(xml)
    ]
  }
  ...
  reqFundamentalData(reqId, contract, reportType, fundamentalDataOptions = null) {
    ...
    let buffer = [
      this._encode(OUTGOING.REQ_FUNDAMENTAL_DATA),
      this._encode(version),
      this._encode(reqId)
    ]
    // send contract fields
  ...
  cancelFundamentalData(reqId) {
    ...
    let version = 1
    let buffer = [
      this._encode(OUTGOING.CANCEL_FUNDAMENTAL_DATA),
      this._encode(version),
      this._encode(reqId)
    ]
  ...
  calculateImpliedVolatility(
    ...
    let version = 2
    let buffer = [
      this._encode(OUTGOING.REQ_CALC_IMPLIED_VOLAT),
      this._encode(version),
      this._encode(reqId)
    ]
    // send contract fields
  ...
  cancelCalculateImpliedVolatility(reqId) {
    ...
    let version = 1
    let buffer = [
      this._encode(OUTGOING.CANCEL_CALC_IMPLIED_VOLAT),
      this._encode(version),
      this._encode(reqId)
    ]
  ...
  calculateOptionPrice(reqId, contract, volatility, underPrice, optionPriceOptions = null) {
    ...
    let version = 2
    let buffer = [
      this._encode(OUTGOING.REQ_CALC_OPTION_PRICE),
      this._encode(version),
      this._encode(reqId)
    ]
    // send contract fields
  ...
  cancelCalculateOptionPrice(reqId) {
    ...
    let version = 1
    let buffer = [
      this._encode(OUTGOING.CANCEL_CALC_OPTION_PRICE),
      this._encode(version),
      this._encode(reqId)
    ]
  ...
  reqMarketDataType(marketDataType) {
    ...
    let version = 1
    return [
      this._encode(OUTGOING.REQ_MARKET_DATA_TYPE),
      this._encode(version),
      this._encode(marketDataType)
    ]
  }
  ...
  reqSecDefOptParams(reqId, underlyingSymbol, futFopExchange, underlyingSecType, underlyingConId) {
    if (this._serverVersion < MIN_SERVER_VER.SEC_DEF_OPT_PARAMS_REQ) {
      return this._error(reqId, BROKER_ERRORS.UPDATE_TWS, 'It does not support reqSecDefOptParams.')
    }
    return [
  ...
  reqSoftDollarTiers(reqId) {
    ...
    return [this._encode(OUTGOING.SOFT_DOLLAR_TIER), this._encode(reqId)]
  }
  ...
  reqPositionsMulti(reqId, account, modelCode) {
    ...
    let version = 1
    return [
      this._encode(OUTGOING.REQ_POSITIONS_MULTI),
      this._encode(version),
      this._encode(reqId),
  ...
  cancelPositionsMulti(reqId) {
    ...
    let version = 1
    let buffer = [
      this._encode(OUTGOING.CANCEL_POSITIONS_MULTI),
      this._encode(version),
      this._encode(reqId)
    ]
  ...
  cancelAccountUpdatesMulti(reqId) {
    ...
    let version = 1
    let buffer = [
      this._encode(OUTGOING.CANCEL_ACCOUNT_UPDATES_MULTI),
      this._encode(version),
      this._encode(reqId)
    ]
  ...
  reqAccountUpdatesMulti(reqId, acctCode, modelCode, ledgerAndNLV) {
    ...
    let version = 1
    return [
      this._encode(OUTGOING.REQ_ACCOUNT_UPDATES_MULTI),
      this._encode(version),
      this._encode(reqId),
      this._encode(acctCode),
      this._encode(modelCode),
  ...
  reqAccountSummary(reqId, group, tags) {
    ...
    let version = 1
    return [
      this._encode(OUTGOING.REQ_ACCOUNT_SUMMARY),
      this._encode(version),
      this._encode(reqId),
      this._encode(group),
      this._encode(tags)
  ...
  cancelAccountSummary(reqId) {
    ...
    let version = 1
    let buffer = [
      this._encode(OUTGOING.CANCEL_ACCOUNT_SUMMARY),
      this._encode(version),
      this._encode(reqId)
    ]
  ...
  verifyRequest(apiName, apiVersion) {
    ...
    let version = 1
    let buffer = [
      this._encode(OUTGOING.VERIFY_REQUEST),
      this._encode(version),
      this._encode(apiName),
      this._encode(apiVersion)
    ]
  ...
  verifyMessage(apiData) {
    ...
    let version = 1
    let buffer = [
      this._encode(OUTGOING.VERIFY_MESSAGE),
      this._encode(version),
      this._encode(apiData)
    ]
  ...
  verifyAndAuthRequest(apiName, apiVersion, opaqueIsvKey) {
    ...
    let version = 1
    let buffer = [
      this._encode(OUTGOING.VERIFY_AND_AUTH_REQUEST),
      this._encode(version),
      this._encode(apiName),
      this._encode(apiVersion),
      this._encode(opaqueIsvKey)
    ]
  ...
  verifyAndAuthMessage(apiData, xyzResponse) {
    ...
    let version = 1
    let buffer = [
      this._encode(OUTGOING.VERIFY_AND_AUTH_MESSAGE),
      this._encode(version),
      this._encode(apiData),
      this._encode(xyzResponse)
    ]
  ...
  subscribeToGroupEvents(reqId, groupId) {
    ...
    let version = 1
    return [
      this._encode(OUTGOING.SUBSCRIBE_TO_GROUP_EVENTS),
      this._encode(version),
      this._encode(reqId),
      this._encode(groupId)
    ]
  }
  ...
  updateDisplayGroup(reqId, contractInfo) {
    ...
    let version = 1
    return [
      this._encode(OUTGOING.UPDATE_DISPLAY_GROUP),
      this._encode(version),
      this._encode(reqId),
      this._encode(contractInfo)
    ]
  }
  ...
  unsubscribeFromGroupEvents(reqId) {
    ...
    let version = 1
    return [
      this._encode(OUTGOING.UNSUBSCRIBE_FROM_GROUP_EVENTS),
      this._encode(version),
      this._encode(reqId)
    ]
  }
  ...
  reqMatchingSymbols(reqId, pattern) {
    ...
    return [this._encode(OUTGOING.REQ_MATCHING_SYMBOLS), this._encode(reqId), this._encode(pattern)]
  }
  ...
  reqFamilyCodes() {
    ...
    return [this._encode(OUTGOING.REQ_FAMILY_CODES)]
  }
  ...
  reqMktDepthExchanges() {
    ...
    return [this._encode(OUTGOING.REQ_MKT_DEPTH_EXCHANGES)]
  }
  ...
  reqSmartComponents(reqId, bboExchange) {
    ...
    return [
      this._encode(OUTGOING.REQ_SMART_COMPONENTS),
      this._encode(reqId),
      this._encode(bboExchange)
    ]
  }
  ...
  reqNewsProviders() {
    ...
    return [this._encode(OUTGOING.REQ_NEWS_PROVIDERS)]
  }
  ...
  reqHistoricalNews(
    ...
    let buffer = [
      this._encode(OUTGOING.REQ_HISTORICAL_NEWS),
      this._encode(reqId),
      this._encode(conId),
      this._encode(providerCode),
  ...
  reqHistogramData(reqId, contract, useRTH, timePeriod) {
    if (this._serverVersion < MIN_SERVER_VER.REQ_HISTOGRAM) {
      return this._error(reqId, BROKER_ERRORS.UPDATE_TWS, 'It does not support histogram requests.')
    }
    return [
  ...
  cancelHistogramData(reqId) {
    ...
    return [this._encode(OUTGOING.CANCEL_HISTOGRAM_DATA), this._encode(reqId)]
  }
  ...
  reqMarketRule(marketRuleId) {
    ...
    return [this._encode(OUTGOING.REQ_MARKET_RULE), this._encode(marketRuleId)]
  }

  reqPnL(reqId, account, modelCode) {
    if (this._serverVersion < MIN_SERVER_VER.PNL) {
      return this._error(reqId, BROKER_ERRORS.UPDATE_TWS, 'It does not support PnL requests.')
    }
    return [
  ...
  cancelPnL(reqId) {
    ...
    return [this._encode(OUTGOING.CANCEL_PNL), this._encode(reqId)]
  }
  ...
  cancelPnLSingle(reqId) {
    ...
    return [this._encode(OUTGOING.CANCEL_PNL_SINGLE), this._encode(reqId)]
  }
